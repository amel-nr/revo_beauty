...
@if($type != 'Seller')
    <div class="row">
        <div class="col-lg-12 pull-right">
            <a href="{{ url('/sample/create')}}" class="btn btn-rounded btn-info pull-right">{{__('Tambahkan Produk Sampel Baru')}}</a>
        </div>
    </div>
@endif
...
<div class="panel">
    ...
    <div class="panel-heading bord-btm clearfix pad-all h-100">
        <h3 class="panel-title pull-left pad-no">{{ __('Produk Sampel') }}</h3>
        <div class="pull-right clearfix">
            <form class="" id="sort_products" action="" method="GET">
                ...
                @endif
                <div class="box-inline pad-rgt pull-left" style="display: none;">
                    <div class="select" style="min-width: 200px;">
                ...
                <div class="box-inline pad-rgt pull-left">
                    <div class="" style="min-width: 200px;">
                        <input type="text" class="form-control" id="search" name="search"@isset($sort_search) value="{{ $sort_search }}" @endisset placeholder="Ketik nama produk sample">
                    </div>
                </div>
            </form>
        </div>
            ...
            <tbody>
               @foreach($products as $key => $product)
                <tr>
                    <td>{{ $key+1}}</td>
                    <td>
                        <a href="{{ route('product', $product->product->slug) }}" target="_blank" class="media-block">
                            <div class="media-left">
                                <img loading="lazy"  class="img-md" src="{{ asset($product->product->thumbnail_img)}}" alt="Image">
                            </div>
                            <div class="media-body">{{ __($product->nama) }}</div>
                        </a>
                    </td>
                    <td>{{ $product->stok }}</td>
                    <td>{{ $product->max_sample }}</td>
                    <td><label class="switch">
                                <input onchange="update_published(this)" value="{{ $product->id }}" type="checkbox" <?php if($product->status_publis == 1) echo "checked";?> >
                    ...
                    <td>
                        <div class="btn-group dropdown">
                            ...
                            <ul class="dropdown-menu dropdown-menu-right">
                                ...
                                <li><a href="{{route('sample.edit', encrypt($product->id))}}">{{__('Ubah')}}</a></li>
                              
                                <li><a onclick="confirm_modal('{{route('sample.destroy', $product->id)}}');">{{__('Hapus')}}</a></li>
                            </ul>
                        </div>
                    </td>
...
@section('script')
    <script type="text/javascript">
        ...
        function update_published(el){
            ...
            $.post('{{ route('sample.published') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    showAlert('success', 'Published  sample products updated successfully');
                }
                else{
                    showAlert('danger', 'Something went wrong');
                }
