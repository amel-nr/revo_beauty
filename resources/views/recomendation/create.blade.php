...
<div class="col-lg col-lg-offset">
    <div class="panel">
        <div class="panel-heading">
            <h3 class="panel-title">{{__('Informasi Rekomendasi Produk')}}</h3>
        </div>
        ...
        <div class="panel-body">
            ...
            @endphp
                <div class="col-lg-4">
                    <input type="radio" name="filter" data-name="Terlaris" id="Terlaris" value="1" {{$filter->filter == 1? "checked":''}} >
                    <label for="Terlaris">Terlaris</label><br>
                    <input type="radio" name="filter" data-name="Brand" id="Brand" value="2" {{$filter->filter == 2? "checked":''}}>
                    <label for="Brand">Brand</label><br>
                    <input type="radio" name="filter" data-name="Kategori" id="Kategori" value="3" {{$filter->filter == 3? "checked":''}}>
                    <label for="Kategori">Kategori</label><br>
                    <input type="radio" name="filter" data-name="Input Manual" id="Manual" value="4" {{$filter->filter == 4? "checked":''}}>
                    <label for="Manual">Input manual</label><br>
                </div>
                <div class="col-lg-8">
                    <div id="rekomendasiBs" style="display:{{$filter->filter == 1?'block':'none'}};">
                        <div style="overflow:scroll">
                            <div id="tblBs"></div>
                        </div>
                    </div>

                    <div id="rekomendasibrand" style="display:{{$filter->filter == 2?'block':'none'}};">                                
                    ...
                    <div id="rekomendasictg" style="display:{{$filter->filter == 3?'block':'none'}};">                                
                            <div class="form-group">
                    ...
                    <div id="manualinput" style="display:{{$filter->filter == 4?'block':'none'}};">
                        <div class="form-group">
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $("input[name='filter']").on("change", function () {
            ...
            $.get("{{route('recomendation.update.filter',['id'=>'1','val'=>'filterval'])}}".replace('filterval',val), function (data) {
        ...
        $("#btnAddBrand").on("click", function (e) {
                ...
                $.post("{{route('add_brand.rekom')}}", data, function (ndata) {
                    fetchBrands()
                })
        })
        ...
        $("#btnAddCtg").on("click", function (e) {
            e.preventDefault()
                ...
                $.post("{{route('add_category.rekom')}}", data, function (ndata) {
                    fetchCategories()
                })
        })
        ...
        $("#btnAddproduk").on("click", function (e) {
            ...
            let id = $("#selectproduk").val()
                ...
                $.post("{{route('add_product.rekom')}}", data, function (ndata) {
                    fetchProduct()
                })
    ...
    function addBestsell() {
        $.get("{{route('add_best_sell.rekom')}}", function (data) {
            if (data =="success") {
                console.log("sukses")
            }
        })
    ...
    function fetchBrands() {
        $.get("{{route('get_brand.rekom')}}", function (data) {
    ...
    function fetchCategories() {
        $.get("{{route('get_categories.rekom')}}", function (data) {
    ...
    function fetchBs() {
        $.get("{{route('get_best_sell.rekom')}}", function (data) {
            let content = `
            <table class="table">
                <thead>
                    <tr>
    ...
    function fetchProduct() {
        $.get("{{route('get_product.rekom')}}", function (data) {
            let content = `
