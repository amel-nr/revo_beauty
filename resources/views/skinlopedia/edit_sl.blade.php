...
@section('content')
    ...
    <a href="{{route('admin.skinlopedia.list')}}" class="panel-title"><i class="fa fa-arrow-left"></i></a>
        <div class="panel">
            ...
            <!--Horizontal Form-->
            <!--===================================================-->
            <form action="{{route('admin.skinlopedia.update',['id'=> $kata->id])}}" method="post" class="form-horizontal" id="formblog" enctype="multipart/form-data">
            {{ csrf_field() }}
            @method('put')
                <div class="panel-body">
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <input type="text" class="form-control" name="title" value="{{$kata->title}}" placeholder="{{__('kata kunci')}}" required>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="col-sm-2 control-label">{{__('Gambar')}}</label>
                            <div class="col-sm-10">
                            <input type="file" name="img" id="img" class="dropify" data-max-file-size="3M" data-allowed-file-extensions="jpg png jpeg" data-default-file="{{asset('skinlopedia/img/').'/'.$kata->img}}">
                            </div>
                    </div>
                    <div class="form-group">
                    <label class="col-sm-2 control-label" for="text">{{__('Definisi')}}</label>
                    <div class="col-sm-10">
                        <textarea class="content" id="definisi" name="text" required>{!!$kata->text!!}</textarea>
                    </div>
                </div>
                </div>
