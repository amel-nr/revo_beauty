...
<div class="col-lg-12">
    <div class="panel">
        <div class="panel-heading">
            <h3 class="panel-title">#{{ $conversation->title }} (Antara @if($conversation->sender != null) {{ $conversation->sender->name }} @endif dan @if($conversation->receiver != null) {{ $conversation->receiver->name }} @endif)
            </h3>
        </div>
            ...
            @if (Auth::user()->id == $conversation->receiver_id)
                <form action="{{ route('messages.store') }}" method="POST">
                    ...
                    <div class="text-right">
                        <button type="submit" class="btn btn-info">{{__('Kirim')}}</button>
                    </div>
                </form>
            @endif
        </div>
