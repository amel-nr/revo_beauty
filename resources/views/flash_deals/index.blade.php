...
<div class="row">
    <div class="col-sm-12">
        <a href="{{ route('flash_deals.create')}}" class="btn btn-rounded btn-info pull-right">{{__('Tambah Flash Sale')}}</a>
    </div>
</div>
...
<div class="panel">
    <div class="panel-heading bord-btm clearfix pad-all h-100">
        <h3 class="panel-title pull-left pad-no">{{__('Flash Sale')}}</h3>
        <div class="pull-right clearfix">
            <form class="" id="sort_flash_deals" action="" method="GET">
                <div class="box-inline pad-rgt pull-left">
                    <div class="" style="min-width: 200px;">
                        <input type="text" class="form-control" id="search" name="search"@isset($sort_search) value="{{ $sort_search }}" @endisset placeholder="Ketik nama flash sale">
                    </div>
                </div>
            </form>
        </div>
            ...
            <tbody>
                @foreach($flash_deals as $key => $flash_deal)
                    <tr>
                        ...
                        <td>{{ date('d-m-Y H:i', $flash_deal->start_date) }}</td>
                        <td>{{ date('d-m-Y H:i', $flash_deal->end_date) }}</td>
                        ...
                        <td>
                            <div class="btn-group dropdown">
                                ...
                                <ul class="dropdown-menu dropdown-menu-right">
                                    <li><a href="{{route('flash_deals.edit', encrypt($flash_deal->id))}}">{{__('Ubah')}}</a></li>
                                    <li><a onclick="confirm_modal('{{route('flash_deals.destroy', $flash_deal->id)}}');">{{__('Hapus')}}</a></li>
                                </ul>
                            </div>
                        </td>
                    </tr>
