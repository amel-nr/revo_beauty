...
if (! function_exists('filter_products')) {
    function filter_products($products) {
        ...
        if(BusinessSetting::where('type', 'vendor_system_activation')->first()->value == 1){
            ...
            if(Auth::check() && Auth::user()->user_tier->is_product_spesial == 1)
            {
                ...
                return $products->where('published', '1')->where('is_special',0)->orderBy('created_at', 'desc')->where(function($p) use ($verified_sellers){
                    $p->where('added_by', 'admin')->orWhere(function($q) use ($verified_sellers){
                        $q->whereIn('user_id', $verified_sellers);
                    });
                });
            ...
            return $products->where('published', '1')->where('is_special',0)->where('added_by', 'admin');
        }
    }
}
...
if(! function_exists('renderStarRating')){
    function renderStarRating($rating,$maxRating=5) {
        $fullStar = "<i class = 'fa fa-heart active' aria-hidden='true' style='margin: 2px; color: #F3795C;'></i>";
        $halfStar = "<i class = 'fa fa-heart half' aria-hidden='true' style='margin: 2px; color: #F3795C;'></i>";
        $emptyStar = "<i class = 'fa fa-heart' aria-hidden='true' style='margin: 2px; color: #FBD2CD;' ></i>";
...
if(! function_exists('renderStarRating2')){
    function renderStarRating2($rating,$maxRating=5) {
        $fullStar = "<i class = 'fa fa-heart active' aria-hidden='true' style='margin: 2px; color: #F3795C;'></i>";
        $halfStar = "<i class = 'fa fa-heart half' aria-hidden='true' style='margin: 2px; color: #F3795C;'></i>";
        $emptyStar = "<i class = 'fa fa-heart-o' aria-hidden='true' style='margin: 2px; color: #F3795C;' ></i>";
        $rating = $rating <= $maxRating?$rating:$maxRating;
...
if (! function_exists('global_request_raja_ongkir')) {

    function global_request_raja_ongkir($url,$tipe = 'GET',$param)
    {
        $curl = curl_init();
...
if (! function_exists('request_mitrans_payment')) {

    function request_mitrans_payment($url,$tipe = 'GET',$param)
    {
        $curl = curl_init();

        curl_setopt_array($curl, array(
        CURLOPT_URL => "https://api.sandbox.midtrans.com/v2/".$url,
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_ENCODING => "",
        CURLOPT_MAXREDIRS => 10,
        CURLOPT_TIMEOUT => 30,
        ...
        CURLOPT_HTTPHEADER => array(
            "Content-Type: application/json",
            "Authorization: Basic ".env('MITRANS_KEY'),
...
if (! function_exists('verify_signature_key')) {

    function verify_signature_key($verify_signature_key,$request)
    {
...
if (! function_exists('image_avatar')) {

    function image_avatar($type)
    {

      if ($type == 'P') {
          # code...
        return asset('frontend/images/avatar/avatar-female.png');

      }else{
        return asset('frontend/images/avatar/avatar-male.png');
      }
