...
class CheckoutController extends Controller
{
    ...
    {
        return view('frontend.credit_card');
    }

    public function get_payment_info(Request $request)
    {
        // dd($request->all());
        $shipping_info = json_decode($request->cost);
        $subtotal = 0;
        $tax = 0;
        ...
        $shipping_info_raw =$request->cost;
        $id_address = $request->id_address;
        return view('frontend.payment_select', compact('total','shipping_info_raw','id_address'));
    }
    ...
    public function apply_coupon_code(Request $request){
        ...
        if($coupon != null){
            ...
            if(strtotime(date('d-m-Y')) >= $coupon->start_date && strtotime(date('d-m-Y')) <= $coupon->end_date){
                ...
                if( $coupon->status_code == 'user_affiliate')
                {
                    $usercode =  AffiliateUserCode::where('kode_id',$request->code)->first();

                    if($usercode->is_active == 1 && strtotime(date('d-m-Y')) > $coupon->end_date)   
                    {
                        $usercode->is_active = 0;
                        $usercode->save();
                        flash('Coupon expired!')->warning();
                        return back();
                    } elseif ( $usercode->is_active = 0) {
                        # code...
                        flash('Coupon expired!')->warning();
                        return back();
                ...
                if ($coupon->status_code == 'user_ultah') {
                    # code...
                    $user = DB::table('users')->where('id',Auth::user()->id)->whereRaw('user_type = "customer" and tgl_lahir is not null and email is not null and DATE_FORMAT(tgl_lahir,"%m%d") = DATE_FORMAT(NOW(),"%m%d")')->first();

                    if($user == null || Auth::user()->user_tier->is_kupon_ultah == 0 )
                    {
    ...
    public function confirmation_payment($id){
        ...
        return view('frontend.confirmation_payment',compact('order'));
    }
